...
from .metrics import (
    ...
    LLM_INPUT_TOKENS_AVG,
    LLM_OUTPUT_TOKENS_AVG,
    EMBEDDING_TOKENS_TOTAL,
    NO_RELEVANT_MEMORY_COUNTER,
    RESPONSE_TIME_SUM,
    RESPONSE_TIME_COUNT,
    RESPONSE_TIME_MAX,
)
...
def _update_llm_stats(model_name, input_tokens, output_tokens):
    if model_name not in _llm_stats:
        _llm_stats[model_name] = {"count": 0, "total_input": 0, "total_output": 0}
...
def fast_reply(message, cat):
    ...
    try:
        ...
        if text:
            # If a fast reply was triggered by the guardian enricher, count it.
            # The guardian sets `cat.working_memory.fast_reply_triggered` when returning
            # a default/early response (which may be localized per browser language).
